...
public class ExplodingProjectile : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if (Missile)
        {
            ...
            projectileSpeed = Mathf.Clamp(projectileSpeed,0,100);
    ...
    void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.tag != "FX")
        {
            ...
            if(collision.gameObject.CompareTag("Enemy")){
                float curHealth = PlayerManager.Instance.player.TakeDamage(PlayerManager.Instance.player,collision.gameObject.GetComponent<EnemyBoat>().state);
